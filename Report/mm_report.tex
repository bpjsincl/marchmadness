...
Additional definitions of a good model include: activation functions and the training function[B1]. % not sure what this scentence if referring to
The adjustment of these parameters varies with every application. % does this go with above
...
The network is built using $buildNewtork$, setting the hidden classes as $TahnLayers$. % <and output class as $SoftMax$.
...
The dataset contains samples from prior seasons <and having $weightdecay$ of non-zero reduces their impact on learned weights.> % don't like this wording
