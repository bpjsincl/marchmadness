...
Comparing the two methods together the Random Forest would consistenly outperform the Neural Network. By leveraging the ensemble learning method, the Random Forest does a good job of predicting the probabilities of each matchup. If multiple decision trees have conflicting results the probabilities are very similar, which is why some probabilities were assigned equal likelihood. For this reason the log loss would generally yield better results in comparison to the Neural Network.
